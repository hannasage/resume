...
/* ============================================================
   RESTRUCTURING AMERICAN SOCCER — A Sage Advice LLC BA Brief · v0.3
   ------------------------------------------------------------
...
function FeeChart({ t }) {
  return (
    <ChartCard t={t} title="The parked-money curve: MLS expansion fees, 1998–2023 ($M)" height={300}
      caption="Entry to Division I is purchased, and the price of scarcity compounds: a 100x escalation in 25 years.">
      <ResponsiveContainer>
...
function NwslFeeChart({ t }) {
  return (
    <ChartCard t={t} title="NWSL expansion fees ($M): the same curve, faster" height={160}
      caption="Denver's $110M (Jan 2025) is the largest fee in US women's sports history, more than double Boston's fee agreed 16 months earlier.">
      <ResponsiveContainer>
...
function YouthCostChart({ t }) {
  return (
    <ChartCard t={t} title="The cost cliff: annual family cost by tier (USD)" height={210}
      caption="Floating bars show reported ranges, all-in (dues + travel + gear + tournaments). The cliff appears at ages 10–12, exactly when serious development begins.">
      <ResponsiveContainer>
...
function SplitDonuts({ t }) {
  ...
  return (
    <Card t={t} style={{ margin: "16px 0" }}>
      ...
      <div style={{ fontFamily: t.font, color: t.muted, fontSize: 11, marginBottom: 8, lineHeight: 1.6, fontWeight: 300 }}>Pre-1992: pooled across all four divisions (50 / 25 / 12.5 / 12.5). Post-breakaway: the top flight retained effectively all of its own new deal, with only minor early solidarity flowing down (illustrative split shown).</div>
...
function ParachuteChart({ t }) {
  return (
    <ChartCard t={t} title="England's distortion: parachute vs. solidarity (£M / club / yr, approx.)" height={200}
      caption="Recently-relegated clubs receive ~8x what their division rivals get; academic work finds they're roughly twice as likely to be promoted, a 'revolving door' at the top of the Championship.">
      <ResponsiveContainer>
...
function UclChart({ t }) {
  return (
    <Card t={t} style={{ margin: "16px 0" }}>
      ...
      <div style={{ fontFamily: t.font, color: t.muted, fontSize: 11, marginBottom: 10, lineHeight: 1.6, fontWeight: 300 }}>10% of gross revenue is committed to solidarity; €308M reached clubs not even in the league phases, the redistribution layer the closed US model lacks entirely.</div>
...
function PrizeChart({ t }) {
  return (
    <ChartCard t={t} title="Open Cup prize money: doubled for 2026 ($K)" height={210}
      caption="Directionally right; still roughly two orders of magnitude below FA Cup distributions. The 2026 edition also adds the John Motta Trophy for the deepest-advancing Open Division side.">
      <ResponsiveContainer>
...
function EquityChart({ t }) {
  ...
  return (
    <ChartCard t={t} title="Why you convert, not refund: MLS franchise value ($B, est.)" height={140}
      caption="~$3.5–4B was ever paid in; ~$18–21B of valuation now sits on top of it. No refund is possible; owners must be converted into shareholders of the thing that actually generates the value.">
      <ResponsiveContainer>
...
function BrazilPyramid({ t }) {
  ...
  return (
    <Card t={t} style={{ margin: "18px 0" }}>
      <div style={{ fontFamily: t.font, color: t.text, fontSize: 13, fontWeight: 700, marginBottom: 2 }}>BRAZIL'S DUAL PYRAMID: national and 27 state systems, joined by gateways</div>
      <div style={{ fontFamily: t.font, color: t.muted, fontSize: 12.5, marginBottom: 14, lineHeight: 1.5 }}>Clubs play both systems every year. State placement feeds Série D and the Copa do Brasil, so any club, in any town, has a defined annual merit path into the national game.</div>
...
function NavBar({ t, tabs, active, setActive, themeId, setThemeId, cameFromResume }) {
  return (
    <div style={{ position: "sticky", top: 0, zIndex: 50, background: t.isDark ? "rgba(7,9,12,.92)" : "rgba(246,248,250,.94)", backdropFilter: "blur(10px)", borderBottom: "1px solid " + t.border }}>
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "10px 12px", display: "flex", alignItems: "center", gap: 8 }}>
        {cameFromResume && (
          <a href="/" aria-label="Back to Hanna Sage's site"
            style={{ display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, borderRadius: t.radius.sm, border: "1px solid " + t.border, padding: "8px 10px", cursor: "pointer", background: "transparent", color: t.muted, minHeight: 40, textDecoration: "none" }}>
            <svg width="15" height="15" viewBox="0 0 14 14" fill="none" aria-hidden style={{ flexShrink: 0 }}>
              <path d="M8.5 2.5 4 7l4.5 4.5" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </a>
        )}
    ...
    <div style={{ position: "relative", overflow: "hidden", background: t.faint, borderBottom: "1px solid " + t.border }}>
      ...
      <div style={{ position: "relative", maxWidth: 728, margin: "0 auto", padding: "40px 16px 32px" }}>
        ...
        <div className="meta-grid" style={{ borderTop: "1px solid " + t.border, borderBottom: "1px solid " + t.border, padding: "14px 0", marginBottom: 22 }}>
          ...
          {metaItem("Version", "V0.3")}
          {metaItem("Last updated", "July 20, 2026")}
  ...
  return (
    <Section t={t} num="03" kicker="PARTS III + III-B" title="Case studies: five footballing nations, read honestly">
      <LessonCard t={t} country="England" flag="🏴󠁧󠁢󠁥󠁮󠁧󠁿" lesson="Jeopardy is monetizable, but write redistribution into the constitution">
        ...
        <ParachuteChart t={t} />
        <P t={t}><Strong t={t}>The WSL is the closest analog to the transition mechanism this plan needs.</Strong> 2011: the FA launches the WSL with 8 teams selected by application (16 applied), a licensed, closed launch, no relegation for two seasons. 2014: WSL 2 created; pro/rel introduced within. 2016: promotion from the wider pyramid opened, and the licensed league was stitched back into the open pyramid. 2018–19: full professionalization by re-licensing: every club re-applied against criteria (16-hour minimum contracts, mandatory academies); Sunderland was denied a license and dropped despite mid-table results; Manchester United (re-formed 2018) entered by application. Controversial, but it worked: fully professional in seven years, and the global standard-setter that forced NWSL's CBA revolution. 2023–25: governance spun out of the FA into a club-owned company (now WSL Football); the top flight expands to 14 for 2026–27.<CiteC t={t} n={[46, 47, 48]} /></P>
...
function S6({ t }) {
  return (
    <Section t={t} num="06" kicker="PART IV-B" title="The Lamar Hunt U.S. Open Cup: full analysis">
      ...
      ]} caption="What the war proved: (a) the USSF bends to MLS in private even after standing firm in public, governance capture quantified; (b) organized public opinion moved the outcome; (c) MLS's objection is economic, not principled, which means it is purchasable." />
      <H3 t={t}>Attempts to become an FA Cup: failed and successful</H3>
      <P t={t}><Strong t={t}>Failed or stalled:</Strong> prize scaling to relevance (even doubled, $600K cannot move an MLS roster decision, since the FA Cup's pull rests on nine figures of distributed value plus a Wembley final); broadcast packaging (repeated USSF efforts never produced a marquee deal, no free-to-air showcase that makes cup upsets national events); scheduling dignity (midweek graveyard slots, finals at the finalist's stadium rather than a neutral cathedral, constant calendar subordination); and the 2023 reserve-team dilution attempt, beaten back, but brand damage the FA Cup has never suffered.</P>
      <P t={t}><Strong t={t}>Successful or promising:</Strong> surviving 1928 and 2023, since institutional persistence is itself the asset, and 100+ editions minus only the COVID years is a heritage moat no new competition can buy; the 2016 reserve-team ban, protecting authenticity a decade before it was tested; the upset economy (Sacramento 2022, the annual amateur "cupset" cycle, a devoted independent media ecosystem where TheCup.us has covered it for decades<CiteC t={t} n={60} />); and the 2025–26 reforms (prize doubling, the Motta Trophy, cleaner Open Division qualifying, coordinated MLS entry), the first coherent USSF investment package in the Cup's modern history.<CiteC t={t} n={61} /> Most important is the USL alignment: with USL Premier launching in 2028, the Cup becomes the venue where the two rival D1s can actually meet. <Strong t={t}>A USL Premier side beating MLS opposition in a well-broadcast Cup run is the single highest-leverage narrative event available to the open-pyramid movement.</Strong> The Cup is where the league war becomes legible to casual fans.</P>
      <PullQuote t={t} attr="Reform package recommended by this brief, feeding the roadmap in Section 07">
        Ring-fenced free-to-air broadcast deal; a permanent neutral-venue final rotated among iconic stadiums; a prize fund tied to a fixed percentage of USSF commercial revenue so it self-scales; guaranteed lower-seed hosting through the Round of 32; and a solidarity kicker, where every Cup gate involving a top-two-division club shares a fixed percentage with the lower-division participant's academy.
      </PullQuote>
    </Section>
  );
}
...
export default function App() {
  ...
  // Offer a "back to site" link only if this visitor actually came from the
  // resume site this session (sessionStorage flag set on the homepage), or
  // arrived via a same-origin referrer. Strangers who open a shared link cold
  // get no path back to the resume. Client-only to avoid hydration mismatch.
  const [cameFromResume, setCameFromResume] = useState(false);
  React.useEffect(() => {
    try {
      const flagged = sessionStorage.getItem("sa:fromResume") === "1";
      const ref = document.referrer;
      const sameOrigin = !!ref && new URL(ref).origin === window.location.origin;
      if (flagged || sameOrigin) setCameFromResume(true);
    } catch { /* storage/URL parsing unavailable — leave link hidden */ }
  }, []);

  // Deep linking: ?section=<id> selects a section on load so any section is a
  // shareable link. The regular page preview covers all of them, so no
  // per-section OG image is needed.
  const didInit = React.useRef(false);
  React.useEffect(() => {
    try {
      const s = new URLSearchParams(window.location.search).get("section");
      if (s && TABS.some((x) => x.id === s)) setActive(s);
    } catch { /* no-op */ }
    didInit.current = true;
    // TABS is rebuilt each render but its ids are stable; run once on mount.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Keep the URL query in sync with the active section so the address bar is
  // always a copyable deep link. replaceState (not push) avoids stacking a
  // history entry per tab. The default section keeps a clean, query-less URL.
  React.useEffect(() => {
    if (!didInit.current || typeof window === "undefined") return;
    const url = new URL(window.location.href);
    if (active === DEFAULT_TAB) url.searchParams.delete("section");
  ...
  return (
    <JumpCtx.Provider value={jump}>
      <div style={{ background: t.bg, minHeight: "100vh", fontFamily: t.font, transition: "background .2s ease" }}>
        <GlobalStyles t={t} />
        <NavBar t={t} tabs={TABS} active={active} setActive={goTab} themeId={themeId} setThemeId={setThemeId} cameFromResume={cameFromResume} />
        ...
        <main style={{ maxWidth: 728, margin: "0 auto", padding: "0 16px" }}>
          {panel.node}
          <PanelFooter t={t} idx={idx} tabs={TABS} goTab={goTab} cameFromResume={cameFromResume} />
        </main>
      </div>
    </JumpCtx.Provider>
  );
  ...
  return (
    <div style={{ borderBottom: "1px solid " + t.border, background: t.faint }}>
      <div style={{ maxWidth: 728, margin: "0 auto", padding: "11px 16px", display: "flex", alignItems: "center", gap: 10 }}>
        <span style={{ fontFamily: t.font, fontSize: 9.5, letterSpacing: "2px", color: t.muted }}>RESEARCH BRIEF · V0.3</span>
...
function PanelFooter({ t, idx, tabs, goTab, cameFromResume }) {
    ...
    <div style={{ borderTop: "1px solid " + t.border, margin: "8px 0 0", padding: "20px 0 46px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        {cameFromResume ? (
          ...
          <span style={{ fontFamily: t.font, fontSize: 9.5, letterSpacing: "2.5px", color: t.muted, fontWeight: 300 }}>
            HANNA SAGE · SAGE ADVICE LLC
          </span>
        )}
      </div>
    </div>
...
/* ------------------------------------------------------------
   Homepage card is driven by this entry in data/analyses.json:
   {
     ...
     "meta": ["research brief", "85 sources", "v0.3"],
     "featured": true
   }
   ------------------------------------------------------------ */
